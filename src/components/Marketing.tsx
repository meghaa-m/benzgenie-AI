...
export default function Marketing() {
  ...
  return (
    <motion.div 
      ...
      className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-140px)] overflow-hidden"
    >
      {/* Left Input form panel */}
      <div className="lg:col-span-5 bg-slate-900/60 p-5 rounded-2xl border border-slate-800/80 flex flex-col justify-between h-full overflow-y-auto pr-1">
        <form onSubmit={handleGenerate} className="space-y-4">
          <div className="flex items-center gap-2 pb-3 border-b border-slate-800/60">
            <Megaphone className="w-5 h-5 text-violet-400 animate-pulse" />
            <div>
              <h1 className="text-base font-display font-bold text-white">BizGenie AI Marketing copywriter</h1>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-1.5 py-2.5 px-2 rounded-xl text-[10px] font-medium border cursor-pointer transition-all ${
                    platform === p.id 
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl py-2.5 px-3 text-white text-xs h-24 focus:outline-none focus:border-violet-500"
            />
          </div>
          ...
          >
            {isGenerating ? 'Compiling copyset copy...' : 'Synthesize Campaign with Gemini'}
        ...
        <div className="mt-6 pt-4 border-t border-slate-800/60">
          <h2 className="text-xs font-mono text-slate-400 uppercase tracking-wider mb-2.5">Saved Campaign Assets</h2>
          <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
            {posts.length === 0 ? (
              <span className="text-[10px] text-slate-500 block italic">No saved posts. Synthesize and save items above.</span>
            ) : (
              posts.map((p) => (
                <div key={p.id} className="p-2.5 bg-slate-950/40 rounded-lg border border-slate-800 flex justify-between items-center">
                  <div className="flex items-center gap-2.5 overflow-hidden">
                    {getPlatformIcon(p.platform)}
                    <div className="overflow-hidden">
                      <h4 className="text-[11px] font-bold text-white truncate max-w-[150px]">{p.topic}</h4>
                  ...
                  <button
                    ...
                    className="text-[9px] font-mono text-violet-400 hover:text-violet-300 font-semibold cursor-pointer shrink-0"
                  >
      ...
      <div className="lg:col-span-7 bg-slate-900/40 rounded-2xl border border-slate-800/60 p-5 flex flex-col justify-between h-full overflow-hidden">
          ...
          {generatedContent && (
            <div className="flex gap-2">
              <button
                ...
                onClick={handleCopy}
                className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs px-2.5 py-1.5 rounded-lg font-medium border border-slate-700 cursor-pointer"
              >
        ...
        {/* Display Container */}
        <div className="flex-grow overflow-y-auto bg-slate-950/40 border border-slate-800/60 rounded-xl p-5 font-mono text-xs text-slate-300 leading-relaxed whitespace-pre-wrap select-text h-full">
          {isGenerating ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
              <Bot className="w-8 h-8 text-violet-400 animate-spin" />
              <div className="space-y-1.5">
                <p className="text-slate-300 font-semibold">Gemini is writing copy...</p>
                <p className="text-[10px] text-slate-500">Formulating custom structural hooks and hashtags</p>
              </div>
            </div>
          ) : generatedContent ? (
            <div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-500">
              ...
              <p className="text-xs font-medium">Your generated marketing copy will print here.</p>
              <p className="text-[10px] text-slate-600 mt-1 max-w-xs">Select target platforms, write brand themes, and click generate above.</p>
            </div>
          )}
        </div>
      </div>
